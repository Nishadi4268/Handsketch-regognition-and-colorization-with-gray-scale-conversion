...
from src.sketch2real import sketch_to_real
import numpy as np
import cv2
import os
...
real_image_path = f"outputs/real_{label}.png"
prompt = f"A realistic {label}, highly detailed, in daylight"

# Call sketch-to-real function
sketch_to_real(input_image, real_image_path, prompt)
...
with col3:
    st.image(colorized_path, caption="Colorized Sketch", use_container_width=True)
with col4:
    st.image(real_image_path, caption=f"Realistic {label}", use_container_width=True)
